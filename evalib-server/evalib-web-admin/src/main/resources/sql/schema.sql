...
(
    ...
    state smallint not null default 0, -- 0: active, 1: inactive
...
drop table if exists t_login_role cascade;
...
(
   ...
   constraint fk_loginrole_login foreign key(login_id) references t_login(id),
   constraint fk_loginrole_role foreign key(role_id) references t_role(id)
);
